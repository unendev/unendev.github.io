...
此MOD从一个简单的静态UI物品，逐步演进为一个支持网络同步的团队计划功能。此过程暴露了我在UI框架、组件生命周期和客户端/服务器架构方面等多个方面理解不足
...
>
> **分析**: 错误源于 `HasItemWithTag` 在一个 `DoTaskInTime` 回调中被调用。该回调执行的时，玩家的 `inventory` 组件尚没有完成初始化，导致 `inst.components.inventory` 为 `nil`
>
> **解决方案**: 改为在玩家的 `OnLoad` 函数或 `ms_playeractivated` 事件监听中，直接遍历 `inst.components.inventory.itemslots` 检查物品，确保在访问 `inventory` 组件时，它处于可用状态
...
第二阶段的目标是实现团队可用的todolist,但在为UI添加可交互的列表和按钮时，遇到了大量UI框架的报错
...
> *   **UI框架**：Klei的 `TEMPLATES` 控件封装深且脆弱。在缺乏合适的教程时，回归基础控件手动构建UI是更可靠的方案
