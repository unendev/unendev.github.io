...
`repository 'https://github.com/your-username/my-obsidian-notes.git' not found`
...
4.  **修改 `.gitmodules` **:
    ...
    `url = git@github.com:your-username/my-obsidian-notes.git`
5.  **修改 `deploy.yml` **:
    使用 `actions/checkout@v4` 内置的 `ssh-key` 参数来加载私钥
...
### 五、规则配置与index文件
#### 5.1 第三次部署失败：部署环境规则
这次报错后查看日志，`build` 成功了，死在`deploy`截断：
...
> 3.  **SSH Key：更安全高效的认证协议** SSH 协议基于非对称加密：私钥安全地保存在本地，公钥部署在远程服务上。这种认证机制不仅比 HTTPS 更安全，也避免了在自动化脚本中反复认证的麻烦